...
def _make_chat(cls: type, settings: LLMGatewaySettings, frames: list[bytes]) -> Any:
    chat = cls(
    ...
    route_transport(chat, transport, transport)
    return chat


def test_converse_chat_stream_surfaces_gateway_cost(llmgw_settings) -> None:
    chat = _make_chat(
        UiPathChatBedrockConverse, llmgw_settings, [*CONVERSE_STREAM_FRAMES, COST_FRAME]
    )
...
def test_anthropic_bedrock_stream_surfaces_gateway_cost(llmgw_settings) -> None:
    chat = _make_chat(
        UiPathChatAnthropicBedrock, llmgw_settings, [*ANTHROPIC_STREAM_FRAMES, COST_FRAME]
    )
    chunks = list(chat.stream("Hello"))
    assert "".join(c.text for c in chunks) == "Hello"
    priced = [c for c in chunks if "associated_dollar_cost" in c.response_metadata]
...
def test_invoke_chat_stream_surfaces_gateway_cost(llmgw_settings) -> None:
    chat = _make_chat(UiPathChatBedrock, llmgw_settings, [*ANTHROPIC_STREAM_FRAMES, COST_FRAME])
    chunks = list(chat.stream("Hello"))
    assert "".join(c.text for c in chunks) == "Hello"
    priced = [c for c in chunks if "associated_dollar_cost" in c.response_metadata]
    assert len(priced) == 1
...
def test_invoke_chat_stream_omits_cost_when_not_priced(llmgw_settings) -> None:
    chat = _make_chat(UiPathChatBedrock, llmgw_settings, ANTHROPIC_STREAM_FRAMES)
    chunks = list(chat.stream("Hello"))
